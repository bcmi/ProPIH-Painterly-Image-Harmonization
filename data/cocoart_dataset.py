...
class COCOARTDataset(BaseDataset):
    ...
    def _load_images_paths(self,):
        ...
        if self.isTrain:
            ...
            random.shuffle(self.path_style)

        else:
            print('loading testing set')
            for img_name in os.listdir(os.path.join(self.opt.content_dir, 'comp')):
                self.path_img.append(os.path.join(self.opt.content_dir, 'comp', img_name))
                self.path_mask.append(os.path.join(self.opt.content_dir, 'mask', img_name.replace('.jpg', '.png')))
            ## style
            for img_name in os.listdir(os.path.join(self.opt.style_dir, 'style')):
                self.path_style.append(os.path.join(self.opt.style_dir, 'style', img_name))
    ...
    def get_small_scale_mask(self, mask, number):
        ...
        mask_patch = np.zeros([number, number],dtype=np.float32)
        split_size = self.opt.load_size // number
        for i in range(number):
            for j in range(number):
                mask_split = mask[i*split_size: (i+1)*split_size, j*split_size: (j+1)*split_size]
                mask_patch[i, j] = (np.sum(mask_split) > 100) * 255
                #mask_patch[i, j] = (np.sum(mask_split) / (split_size * split_size)) * 255
        mask_patch = np.uint8(mask_patch)
        return Image.fromarray(mask_patch,mode='L')
    ...
    def __getitem__(self, index):
        if self.isTrain:
            ...
            np_mask = cv2.imread(self.path_mask[index], 0)
        
        soft_mask = cv2.blur(np.array(np_mask), (7, 7))
        mask = Image.fromarray(np_mask,mode='L')
        soft_mask = Image.fromarray(soft_mask,mode='L')
        ...
        soft_mask = tf.resize(soft_mask, [self.opt.load_size, self.opt.load_size])
        mask_patch = self.get_small_scale_mask(mask, self.opt.patch_number)
        ...
        mask = mask*2 -1
        soft_mask = tf.to_tensor(soft_mask)
        soft_mask = soft_mask*2 -1
        mask_patch = tf.to_tensor(mask_patch)
        comp = self._compose(content, mask, style)

        return {'comp': comp, 'mask': mask, 'mask_patch': mask_patch, 'soft_mask': soft_mask, 'style': style,'content':content, 'img_path':self.path_style[index]}
